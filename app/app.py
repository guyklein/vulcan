...
def get_all_plugins():
    """
    Get all the plugins from the DB, order them by the query params and return the result as an HTML table
...
def get_plugin_by_plugin_id(plugin_id):
    ...
    length = results.count()
    LOG.debug(f'Request to get {plugin_id} was submitted. Result len is {length}')
    response.content_type = 'application/json'
    response.body = json.dumps([r.serialize for r in results.all()])
    return response
...
def get_plugins_by_cve_ids():
    ...
    length = results.count()
    LOG.debug(f'Request to get {cve_ids} was submitted. Result len is {length}')
    response.content_type = 'application/json'
    response.body = json.dumps([r.serialize for r in results.all()])
